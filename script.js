...
function numPages(){
    return Math.ceil(movies.length/recordsPerPage)
}
function sortD(s){
    if(s==null){
        return;
    }
    sortCheck=s.options[s.selectedIndex].text;
...
var sortCheck, order,combinedRating;
...
function makeSort(){
    ...
    }else if(sortCheck==="AverageRating"){                    
        if(order === "A-Z"){
            movies.forEach(a => {
            a.Ar=a.imdb.rating * a.tomatoes.viewer.rating;                        
            });
            movies.sort((a, b) => {
                return b.Ar - a.Ar;
            });                                                                                
...
function changePage(page){
    ...
    var listing_tables = document.getElementById("row")
    var pageSI = document.getElementById("SI")
    var count = 1
    if(page<1)
    page = 1;
    if(page>numPages())
    ...
    listing_tables.innerHTML = ""
    listing_tables.innerHTML += `<tr>
<th>Sl No.</th>
<th>Title</th>
<th>Imdb</th>
<th>Tomatoes</th>
...
<th>View</th>   
     ...
     for (var i = (page-1) * recordsPerPage; i < (page * recordsPerPage) && i < movies.length; i++)  {
    listing_tables.innerHTML+=`<tr>
    <td>${count} </td>
    <td>${movies[i].title} </td>
    <td>${movies[i].imdb.rating } </td>
    <td> ${movies[i].tomatoes.viewer.rating}  </td>
    <td>${((movies[i].imdb.rating + movies[i].tomatoes.viewer.rating*2)/2).toFixed(1)} </td> 
    <td>${movies[i].plot}  </td>
    </tr>`;
    count=count+1;
    }
    ...
    pageSI.innerHTML = page + "/" + numPages();
        if (page == 1) {
